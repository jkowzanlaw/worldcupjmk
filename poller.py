...
def format_kickoff_et(utc_str):
    ...
    return f"{hour}:{et.strftime('%M')} {'AM' if et.hour < 12 else 'PM'} ET"
...
def make_schedule_card(matches,date_str,day_num,output_path):
    ...
    for i,m in enumerate(matches):
        ...
        if st:
            ...
            d.text((S-pad_x-sw-8,ym-(sb[3]-sb[1])//2),st,font=gf,fill=(160,122,40))
        if i<n-1: d.rectangle([pad_x+5,y+row_h-1,S-pad_x,y+row_h],fill=(220,218,212))
    d.rectangle([0,S-FOOTER,S,S],fill=NAVY); d.rectangle([0,S-FOOTER,S,S-FOOTER+4],fill=GOLD_DK)
    centered(d,"WORLD CUP IN 5  ·  2026  ·  ALL TIMES ET",f_br,S-42,S,W)
    Path(output_path).parent.mkdir(parents=True,exist_ok=True)
...
def claude_result_caption(home, away, hs, as_, stage, insight, hashtags):
    prompt = f"""Write an Instagram caption for this World Cup result. Follow this EXACT format:

RESULT: {home} {hs}–{as_} {away} | {stage}
KEY INSIGHT: {insight}

FORMAT TO USE:
Line 1: Bold result statement with score (use emoji ⚽ or 🔥 once max)
Line 2: One sharp tactical or storyline observation
Line 3: One forward-looking line (what this means for the tournament)
[blank line]
{hashtags}

Rules:
- Max 150 words before hashtags
- No clichés like "what a game" or "incredible match"  
- Sound like an expert who watched every minute
- First line must stop the scroll"""
    return claude_call(prompt, max_tokens=400)

def claude_schedule_caption(matches, date_str, day_num, hashtags):
    match_lines = "\n".join([f"- {m['time']}: {m['home']} vs {m['away']} ({m.get('group',m.get('stage',''))})" for m in matches])
    prompt = f"""Write an Instagram caption for a World Cup daily schedule post. Follow this EXACT format:

DATE: {date_str} — Day {day_num} of the 2026 FIFA World Cup
MATCHES TODAY:
{match_lines}

FORMAT TO USE:
Line 1: Punchy opener that creates urgency/excitement (use 1 emoji max)
Line 2-3: Call out the 1-2 most compelling matchups with ONE reason each why they matter
Line 4: CTA — tell followers to save this post or turn on notifications
[blank line]
{hashtags}

Rules:
- Max 120 words before hashtags
- Make casual fans feel like they'd be missing out if they don't tune in
- No generic "great day of football" filler"""
