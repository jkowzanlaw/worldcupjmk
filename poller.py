...
def recap_done_today():
    return (TMP_DIR / f"recap_{datetime.now(timezone.utc).strftime('%Y%m%d')}.done").exists()

def mark_recap_done():
    (TMP_DIR / f"recap_{datetime.now(timezone.utc).strftime('%Y%m%d')}.done").touch()
...
def job_recap_card():
    ...
    try:
        ...
        # Generate card
        img_path = str(TMP_DIR / f"recap_{today_et.strftime('%Y%m%d')}.png")
