...
def get_todays_matches():
    """Fetch all WC matches whose ET kickoff date = today ET.
    Queries UTC date range that covers full ET day:
      ET midnight = UTC+4h, ET 11:59pm = UTC next day +4h
    """
    today_et = et_now()
    today_str = today_et.strftime("%Y-%m-%d")

    # ET day spans from 04:00 UTC to 04:00 UTC next day
    # So query dateFrom = today ET date, dateTo = today ET date + 1 (UTC)
    utc_date_start = today_str  # ET midnight = 04:00 UTC same date
    utc_date_end = (today_et + timedelta(days=1)).strftime("%Y-%m-%d")

    matches = fd_get(
        f"/competitions/WC/matches?dateFrom={utc_date_start}&dateTo={utc_date_end}"
    ).get("matches", [])

    # Filter: only include matches whose ET kickoff date = today
    result = []
    for m in matches:
        m_et = utc_to_et(m["utcDate"])
        if m_et.strftime("%Y-%m-%d") == today_str:
            result.append(m)

    log.debug(f"get_todays_matches: {today_str} ET → {len(result)} matches")
    return result
